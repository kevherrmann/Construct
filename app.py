...
# E-Mail und Teile-Beschaffung gibt es nur in der Vollversion. Der Import steht
# unter der Abfrage, damit make-lite.sh die beiden Dateien schlicht weglassen
# kann — eine weitergegebene Kopie enthält den Apparat dann gar nicht erst.
# Benutzt werden sie ausschließlich in Routen, die LITE_BLOCKED sperrt.
if cfg.LITE:
    mailmod = partsmod = None
else:
    import mail as mailmod  # E-Mail: IMAP/SMTP für GMX, Gmail, Outlook (mail.py)
    import parts as partsmod  # Teile-Beschaffung: Cody sucht, parts.py rechnet
...
LITE_BLOCKED = ("/api/skill", "/api/mcp", "/api/mail", "/api/parts")
...
# ---------- Teile-Beschaffung ----------
# Cody sucht die Produkte und schreibt seine Funde ueber /api/parts/offer
# zurueck -- er laeuft im selben Container, localhost reicht. parts.py rechnet
# daraus die guenstigste Aufteilung inkl. Versand.

def _parts_conn():
    return partsmod.connect()


def _check_url(url: str) -> tuple[bool, str]:
    """Ist der Produktlink echt? -> (ok, Begruendung)

    Nicht aus Misstrauen, sondern aus Erfahrung: ein zusammengebauter Link sieht
    aus wie ein echter, fuehrt aber ins Leere. Und ein Link, den man nicht
    anklicken kann, ist beim Bestellen genauso wertlos wie ein ausverkauftes
    Produkt.

    Bewusst nur harte Faelle (404/410, kein Host, Timeout) gelten als Fehler.
    403/429 heisst meist "Shop mag keine Bots" -- das darf keinen echten Fund
    wegwerfen.
    """
    if not url:
        return True, ""
    try:
        ...
        if e.code in (404, 410):
            return False, f"Link tot (HTTP {e.code})"
        return True, ""              # 403/429/5xx -> nicht verifizierbar
    except Exception as e:
        return False, f"Link nicht erreichbar ({type(e).__name__})"


def _shop_stock(url: str) -> tuple[bool | None, str]:
    """Lieferbarkeit direkt beim Shop erfragen, wo das ohne Raten geht.

    Jeder Shopify-Shop liefert unter `/products/<handle>.js` den Lagerstatus je
    Variante. Das ist Fakt statt Einschaetzung -- und noetig, weil bei
    AZ-Delivery aktuell nur ~10 % des Katalogs ueberhaupt lieferbar sind.
    Varianten mit `sku == "ebook"` sind Gratis-Beigaben, keine Ware.

    -> (True/False, Statustext) oder (None, "") wenn der Shop nichts hergibt.
    """
    m = re.match(r"^(https?://[^/]+)/products/([^/?#]+)", url or "")
    if not m:
        return None, ""                       # kein Shopify-Shop
    base, handle = m.groups()
    try:
        req = urllib.request.Request(
    ...
    varianten = [v for v in data.get("variants", []) if v.get("sku") != "ebook"]
    if not varianten:
        return None, ""
    lieferbar = [v for v in varianten if v.get("available")]
    if lieferbar:
        return True, (f"laut Shop auf Lager ({len(lieferbar)} von "
                      f"{len(varianten)} Varianten)")
    return False, "laut Shop ausverkauft (keine Variante lieferbar)"


@app.get("/api/parts/projects")
def parts_projects():
    conn = _parts_conn()
    rows = conn.execute(
        """SELECT p.slug, p.name, COUNT(t.id) AS teile,
                  SUM(CASE WHEN EXISTS(SELECT 1 FROM offer o WHERE o.part_id=t.id)
                           THEN 1 ELSE 0 END) AS mit_angebot
           FROM project p LEFT JOIN part t ON t.project_id = p.id
           GROUP BY p.id ORDER BY p.created_at DESC""")
    return [dict(r) for r in rows]


@app.post("/api/parts/project")
async def parts_project_create(req: Request):
    body = await req.json()
    ...
    try:
        return partsmod.create_project(_parts_conn(), slug, name or slug, text)
    except ValueError as e:
        return JSONResponse({"error": str(e)}, status_code=400)


@app.get("/api/parts/project/{slug}")
def parts_project(slug: str):
    d = partsmod.load_project(_parts_conn(), slug)
    if d is None:
        return JSONResponse({"error": f"'{slug}' gibt es nicht"}, status_code=404)
    return d


@app.delete("/api/parts/project/{slug}")
def parts_project_del(slug: str):
    conn = _parts_conn()
...
@app.post("/api/parts/offer")
async def parts_offer_add(req: Request):
    """Codys Fundmeldung. Shop wird nebenbei angelegt, falls neu."""
    b = await req.json()
    conn = _parts_conn()
    try:
        # Erst pruefen, dann anlegen -- sonst hinterlaesst eine abgelehnte
        # Meldung einen Shop-Eintrag, den nie jemand bestellt.
        ...
        # Link gegenpruefen, bevor irgendwas gespeichert wird. Die Fehlermeldung
        # geht als curl-Antwort direkt an Cody zurueck -- er kann im selben Lauf
        # die richtige URL nachliefern.
        ok, grund = await asyncio.to_thread(_check_url, (b.get("url") or "").strip())
        if not ok:
            return JSONResponse(
                {"error": f"{grund}: {b.get('url')}. Bitte die echte Produktseite "
                          "aufrufen und deren URL aus der Adresszeile verwenden — "
                          "nicht aus Shop-Name und Artikelbezeichnung zusammenbauen."},
                status_code=400)

        # Wo der Shop den Lagerstand selbst verraet, gilt der -- nicht Codys
        # Einschaetzung. Ein ausverkauftes Produkt kommt gar nicht erst rein.
        url = (b.get("url") or "").strip()
        lager, lager_text = await asyncio.to_thread(_shop_stock, url)
        if lager is False:
            return JSONResponse(
                {"error": f"{lager_text}: {url}. Bitte ein lieferbares Produkt "
                          "suchen — notfalls in einem anderen Shop."},
                status_code=409)
        if lager is True:
            b["available"] = True
            b.setdefault("stock_text", "") or b.update(stock_text=lager_text)

        shop_id = partsmod.upsert_shop(
            conn, (b.get("shop") or "").strip(), b.get("shop_url", ""),
            float(b.get("shipping") or 0),
            None if b.get("free_shipping_at") in (None, "") else float(b["free_shipping_at"]),
            float(b.get("min_order") or 0))
        oid = partsmod.add_offer(
            conn, int(b["part_id"]), shop_id, (b.get("title") or "").strip(),
            float(b["price"]), b.get("url", ""), b.get("sku", ""),
            int(b.get("pack_size") or 1), b.get("note", ""),
            b.get("available", True), b.get("stock_text", ""))
        return {"ok": True, "offer_id": oid}
    except (KeyError, ValueError, TypeError) as e:
        return JSONResponse({"error": f"{type(e).__name__}: {e}"}, status_code=400)


@app.patch("/api/parts/offer/{oid}")
async def parts_offer_patch(oid: int, req: Request):
    b = await req.json()
    sets, vals = [], []
    for feld, key in (("price", "price"), ("pack_size", "pack_size"),
                      ("confirmed", "confirmed"), ("note", "note"),
                      ("available", "available"), ("stock_text", "stock_text")):
        if key in b and b[key] is not None:
            v = b[key]
            if key == "price":
                v = float(v)
                if v <= 0:
                    return JSONResponse({"error": "Preis muss > 0 sein"}, status_code=400)
            if key == "pack_size":
                v = max(int(v), 1)
            if key in ("confirmed", "available"):
                v = int(bool(v))
            sets.append(f"{feld}=?")
            vals.append(v)
    if not sets:
    ...
    conn = _parts_conn()
    conn.execute(f"UPDATE offer SET {','.join(sets)} WHERE id=?", [*vals, oid])
    conn.commit()
    return {"ok": True}


@app.delete("/api/parts/offer/{oid}")
def parts_offer_del(oid: int):
    conn = _parts_conn()
    conn.execute("DELETE FROM offer WHERE id=?", (oid,))
    conn.commit()
    return {"ok": True}


@app.patch("/api/parts/part/{pid}")
async def parts_part_patch(pid: int, req: Request):
    b = await req.json()
    sets, vals = [], []
    if "optional" in b:
        sets.append("optional=?")
        vals.append(int(bool(b["optional"])))
    if b.get("qty"):
        sets.append("qty=?")
        vals.append(max(int(b["qty"]), 1))
    if "note" in b:
        sets.append("note=?")
        vals.append(b["note"] or "")
    if not sets:
        return JSONResponse({"error": "nichts zu ändern"}, status_code=400)
    conn = _parts_conn()
    conn.execute(f"UPDATE part SET {','.join(sets)} WHERE id=?", [*vals, pid])
    conn.commit()
    return {"ok": True}


@app.delete("/api/parts/part/{pid}")
def parts_part_del(pid: int):
    conn = _parts_conn()
...
@app.patch("/api/parts/shop/{key}")
async def parts_shop_patch(key: str, req: Request):
    """Versandkosten und Freigrenzen -- die entscheiden den Vergleich staerker
    als jeder Stueckpreis, also muss Kevin sie korrigieren koennen."""
    b = await req.json()
    conn = _parts_conn()
    sets, vals = [], []
    for feld in ("shipping", "min_order"):
        if b.get(feld) is not None:
            sets.append(f"{feld}=?")
            vals.append(max(float(b[feld]), 0))
    if "free_shipping_at" in b:
        sets.append("free_shipping_at=?")
        vals.append(None if b["free_shipping_at"] in (None, "") else float(b["free_shipping_at"]))
    if not sets:
        return JSONResponse({"error": "nichts zu ändern"}, status_code=400)
    conn.execute(f"UPDATE shop SET {','.join(sets)} WHERE key=?", [*vals, key])
    conn.commit()
    return {"ok": True}


@app.post("/api/parts/recheck/{slug}")
async def parts_recheck(slug: str):
    """Gespeicherte Angebote nachpruefen: Link noch da, Ware noch lieferbar?

    Preise und Lagerstaende veralten. Ohne das faellt es erst beim Bestellen
    auf -- und dann ist die ganze Rechnung Makulatur.
    """
    conn = _parts_conn()
    d = partsmod.load_project(conn, slug)
    if d is None:
        return JSONResponse({"error": f"'{slug}' gibt es nicht"}, status_code=404)

    angebote = [(o["id"], o["url"], o["shop"], o["title"], bool(o["available"]))
                for p in d["parts"] for o in p["offers"]]

    async def pruefe(oid, url, shop, titel, war_ok):
        if not url:
            return None
        link_ok, grund = await asyncio.to_thread(_check_url, url)
        if not link_ok:
            return oid, False, grund, shop, titel, war_ok
        lager, text = await asyncio.to_thread(_shop_stock, url)
        if lager is None:
            return None                      # nicht maschinell pruefbar
        return oid, lager, text, shop, titel, war_ok

    ergebnisse = [r for r in await asyncio.gather(
        *(pruefe(*a) for a in angebote)) if r]

    geaendert = []
    for oid, ok, text, shop, titel, war_ok in ergebnisse:
        conn.execute("UPDATE offer SET available=?, stock_text=? WHERE id=?",
                     (int(ok), text, oid))
        if ok != war_ok:
            geaendert.append({"shop": shop, "titel": titel,
                              "jetzt": "lieferbar" if ok else "nicht lieferbar",
                              "grund": text})
    conn.commit()
    return {"geprueft": len(ergebnisse), "gesamt": len(angebote),
            "geaendert": geaendert}


@app.get("/api/parts/plan/{slug}")
def parts_plan(slug: str, max_shops: int | None = None, include_optional: bool = True,
               ignore_min_order: bool = False, alternatives: int = 3):
    try:
        r = partsmod.plan(_parts_conn(), slug, max_shops, include_optional,
                          ignore_min_order, alternatives)
    except ValueError as e:
        return JSONResponse({"error": str(e)}, status_code=400)
    if r is None:
        return JSONResponse({"error": f"'{slug}' gibt es nicht"}, status_code=404)
...
@app.get("/api/parts/export/{slug}")
def parts_export(slug: str, max_shops: int | None = None, include_optional: bool = True,
                 ignore_min_order: bool = False):
    r = partsmod.plan(_parts_conn(), slug, max_shops, include_optional,
                      ignore_min_order, 1)
    if r is None:
        return JSONResponse({"error": f"'{slug}' gibt es nicht"}, status_code=404)
    sol = r["solutions"][0]
    if not sol["baskets"]:
        return JSONResponse({"error": "Keine vollständige Lösung"}, status_code=400)
    md = partsmod.to_markdown(sol, f"Einkaufsliste {slug}")
    return Response(md, media_type="text/markdown; charset=utf-8",
                    headers={"Content-Disposition":
                             f'attachment; filename="{slug}-einkaufsliste.md"'})


@app.get("/api/parts/prompt/{slug}")
def parts_prompt(slug: str, only_open: bool = True):
    """Der Rechercheauftrag an Cody.

    Bewusst als fertiger Prompt statt als versteckter Systemaufruf: Kevin sieht
    im Chat, was gesucht wird, kann nachfassen ("nimm lieber AliExpress") und
    die Suche jederzeit stoppen. Cody schreibt seine Funde per curl zurueck --
    er laeuft im selben Container, also reicht localhost.
    """
    d = partsmod.load_project(_parts_conn(), slug)
    if d is None:
        return JSONResponse({"error": f"'{slug}' gibt es nicht"}, status_code=404)

    # "Erledigt" heisst: es gibt ein LIEFERBARES Angebot. Ein Teil, fuer das nur
    # ausverkaufte Treffer in der DB stehen, ist offen -- sonst sucht niemand
    # nach, wenn ein Shop sein Lager leerraeumt.
    offen = [p for p in d["parts"]
             if not (only_open and any(o["available"] for o in p["offers"]))]
    if not offen:
        return {"prompt": "", "offen": 0}

    zeilen = "\n".join(
        f"- part_id={p['id']} | {p['qty']}× {p['name']}"
        + (f" [{p['spec']}]" if p["spec"] else "")
        + (" (optional)" if p["optional"] else "")
        for p in offen)

    bekannt = ", ".join(s["name"] for s in d["shops"]) or "noch keine"
    port = os.getenv("MATRIX_PORT", "8765")

    prompt = f"""Suche für das Projekt „{d['name']}" die folgenden Bauteile — \
so günstig wie möglich, in so wenig Shops wie möglich, Lieferung nach Deutschland.

{zeilen}

Bereits benutzte Shops (bevorzugt weiterverwenden, das spart Versand): {bekannt}

## Wichtigste Regel: nur LIEFERBARE Produkte

Ein Link auf ein ausverkauftes Produkt ist wertlos. Deshalb:

- Öffne für JEDEN Fund die **Produktseite** und lies dort den Lieferstatus
  ("ab Lager", "1–2 Werktage", "auf Lager", "sofort lieferbar" → lieferbar;
  "nicht verfügbar", "ausverkauft", "derzeit nicht lieferbar", "Nachfrage",
  ein Verfügbarkeitsdatum in der Zukunft → NICHT lieferbar).
- Steht da nichts Eindeutiges, gilt es als nicht bestätigt: `"available": false`.
  Nicht raten und nicht schönreden.
- Ist ein Produkt nicht lieferbar, **such weiter** statt es zu melden. Erst wenn
  du in keinem Shop etwas Lieferbares findest, melde das beste nicht lieferbare
  mit `"available": false` — dann weiß ich wenigstens, dass es das Teil gibt.
- Den gelesenen Status immer wörtlich in `stock_text` mitgeben.

## Shop-Steckbrief (nachgemessen am 15.08.2026)

| Shop | Suche | Besonderheit |
|---|---|---|
| **reichelt** | `reichelt.de/de/de/shop/suche/<begriff>` | statisches HTML, bestens auslesbar. Breitestes Sortiment: Schrauben, Muttern, LiPo, XT60, Getriebemotoren, Raspberry Pi, Arduino |
| **BerryBase** | `berrybase.de/search?search=<begriff>` | auslesbar; Produktkacheln als `product-box`, Trefferzahl steht im HTML als `"siteSearchResults":N`. Stark bei Raspberry Pi |
| **Eckstein** | `eckstein-shop.de/search?sSearch=<begriff>` | auslesbar, günstige China-Module, Lieferstatus im Klartext auf der Produktseite |
| **Pollin** | `pollin.de/search?query=<begriff>` | auslesbar, gut bei Motoren/Mechanik |
| **AZ-Delivery** | `az-delivery.de/products.json?limit=250&page=N` | **VORSICHT: nur ~10 % des Katalogs lieferbar** (51 von 505 echten Produkten). Immer `variants[].available` prüfen. `sku=="ebook"` sind Gratis-Beigaben, keine Ware |
| **Conrad** | — | **liefert 403, Bot-Schutz.** Nicht auslesbar, Zeit sparen |

**Zwei maschinenlesbare Wege zum Lagerstatus — die schlagen jedes Textraten:**

1. Fast jede Produktseite trägt JSON-LD mit `"availability": "https://schema.org/InStock"`
   (bzw. `OutOfStock`). Danach im HTML suchen, das ist eindeutig.
2. Jeder Shopify-Shop (AZ-Delivery u.a.) liefert unter `/products/<handle>.js`
   den Lagerstand je Variante als `available: true/false`.

Der Server prüft Weg 2 beim Eintragen ohnehin nach und lehnt Ausverkauftes ab —
verlass dich nicht darauf, sondern prüfe vorher, das spart dir den Fehlversuch.
Antwortet ein Shop mit 403/503 (Amazon, Conrad), kannst du die Verfügbarkeit
nicht ehrlich prüfen: dann diesen Shop nicht verwenden.

## Weiteres Vorgehen

1. Preis auf der Produktseite prüfen, nicht in der Trefferliste.
2. Bündeln: ein Shop mehr kostet 5–7 € Versand. Ein Teil für 2 € beim siebten
   Händler kostet real 8 €. Lieber etwas teurer im schon benutzten Shop.
3. Packungsgrößen als `pack_size` eintragen (Schrauben kommen im 100er-Pack),
   nicht den Stückpreis.
4. Unsicherheiten in `note`: abweichendes Maß, unklare Variante, Klon statt
   Original. Lieber ein ehrlicher Hinweis als ein falsches Teil im Warenkorb.

## Rückmeldung — pro Fund ein Aufruf

curl -s -X POST http://127.0.0.1:{port}/api/parts/offer \\
  -H 'Content-Type: application/json' -d '{{
    "part_id": 123, "shop": "reichelt", "shop_url": "https://www.reichelt.de",
    "shipping": 5.95, "free_shipping_at": null, "min_order": 0,
    "title": "M3x10 Zylinderschraube verzinkt, 100er-Pack",
    "sku": "CIMCO 190800", "url": "https://www.reichelt.de/...",
    "price": 4.20, "pack_size": 100,
    "available": true, "stock_text": "ab Lager, Lieferzeit 1-2 Werktage",
    "note": ""
  }}'

`price` = Preis pro Packung. `shipping`/`free_shipping_at` sind Shop-Konditionen,
einmal pro Shop genügt. Melde jeden Fund sofort, nicht gesammelt am Ende.

**Die URL wird serverseitig geprüft.** Nimm die Adresse aus der Adresszeile der
Produktseite, die du wirklich geöffnet hast — baue sie nie aus Shop-Name und
Artikelbezeichnung zusammen. Kommt „Link tot" zurück, hast du geraten: die echte
Produktseite suchen und den Fund mit der richtigen URL erneut melden.

Zum Schluss eine kurze Zusammenfassung: was gefunden, was nicht, wo ich zweimal
hinschauen sollte. Die Preisrechnung macht der Optimizer — trag nur die Funde ein."""

    return {"prompt": prompt, "offen": len(offen)}


# Laufende Recherchen je Projekt. Der Lauf haengt am normalen Run-Unterbau,
# also funktioniert Reconnect, Abbrechen und Backlog-Replay wie im Chat --
# nur dass die Ausgabe im Teile-Modul landet statt in einer Session.
PARTS_RUNS: dict[str, str] = {}


@app.post("/api/parts/search/{slug}")
async def parts_search_start(slug: str, req: Request):
    body = await req.json() if await req.body() else {}
    only_open = body.get("only_open", True) is not False

    laeuft = RUNS.get(PARTS_RUNS.get(slug, ""))
    if laeuft and not laeuft.done:
        return {"run_id": laeuft.id, "schon_am_laufen": True}

    p = parts_prompt(slug, only_open)
    if isinstance(p, JSONResponse):
        return p
    if not p["offen"]:
        return {"run_id": None, "offen": 0}

    run = start_run(p["prompt"], DEFAULT_CWD, "bypassPermissions")
    PARTS_RUNS[slug] = run.id
    return {"run_id": run.id, "offen": p["offen"]}


@app.get("/api/parts/search/{slug}")
def parts_search_state(slug: str):
    """Damit die Ansicht nach einem Reload wieder andockt statt neu zu suchen."""
    rid = PARTS_RUNS.get(slug)
    run = RUNS.get(rid) if rid else None
    return {"run_id": rid if run else None, "running": bool(run and not run.done)}
